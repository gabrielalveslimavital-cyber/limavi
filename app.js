...
// Interakids = azul, Particular = verde-teal, UNG = laranja
const CONVENIOS = {
...
let agendaModoSel  = false;          // true = modo seleção ativo
const agSelecionados = new Set();    // IDs selecionados
...
function renderAgenda() {
  ...
  // Barra de ações em lote (mostra/oculta)
  ...
  el.innerHTML = pacIds.map(pacId => {
    ...
    // Checkbox "selecionar todos deste paciente"
      ...
      <div class="pac-grupo">
        ...
        <div class="pac-grupo-sessoes" id="grupo-${pacId}">
          ${sessoes.map(ag => {
            ...
            return `
              <div class="sessao-item ${!isAtivo?'sessao-finalizada':''} ${sel?'sessao-selecionada':''}"
  ...
  // Reabre grupos que estavam abertos
...
// ── Controle do modo seleção
function ativarModoSelecao() {
  agendaModoSel = true;
  agSelecionados.clear();
  renderAgenda();
  // Abre todos os grupos para facilitar seleção
...
function toggleSessaoSel(agId) {
  ...
  document.querySelectorAll('.sessao-item').forEach(el => {
    // encontra pelo checkbox dentro
    const cb = el.querySelector('.sel-check');
    if (!cb) return;
    // reconstrói classe pela presença do checkbox marcado
  });
  renderAgenda();
}
...
function editarHorarioLote() {
  ...
  // Abre modal de edição de horário em lote
...
function renderEvolucoes() {
  ...
  el.innerHTML = pacIds.map(pacId => {
    ...
    const cor  = convenioCor(pac?.convenio);
    const nome = pac?.nome || 'Paciente não encontrado';
    // Ordena evoluções por data decrescente (mais recente primeiro)
...
function editarEvolucao(id) {
  ...
  // Campos legado (compatibilidade com evoluções antigas)
...
function salvarEvolucao() {
  ...
  const obj = {
    ...
    // Campos legado mantidos para compatibilidade com registros antigos
    subj:  descricao,
    texto: descricao,
    sessao: '',
    eva: null,
...
function salvarProfissional() {
  ...
  renderProfissionais();
  // Atualiza select de profissionais em modais abertos
  populateSelects();
...
// Preenche todos os selects de paciente e profissional nos modais
function populateSelects() {
  ...
  // Select de profissionais — SEMPRE popula com lista atualizada
  const profs = DB.get('profissionais');
  ...
  if (agProfEl) {
    ...
    // Mantém valor existente ou define o usuário logado como padrão
