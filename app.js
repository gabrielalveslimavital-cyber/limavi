...
// Pilha de páginas visitadas para o botão "voltar"
const navHistory = [];
...
function navigate(page, pushHistory = true) {
  ...
  // Empilha página anterior no histórico (mas não duplica o topo)
  if (pushHistory) {
    const current = navHistory[navHistory.length - 1];
    if (current && current !== page) navHistory.push(current);
    else if (!current) navHistory.push(page); // primeiro acesso
  }
  ...
  // Atualiza título do header e botão voltar
...
function goBack() {
  // Remove página atual do histórico
  navHistory.pop();
...
function updateBackButton(page) {
  ...
  if (!btn) return;
  // Mostra botão voltar em qualquer página fora do dashboard
...
const STATUS_ATIVOS = ['agendado', 'confirmado', 'realizado', 'falta'];
// ─── PACIENTES ─────────────────────────────────────────────────────────────
...
function editarPaciente(id) {
  ...
  set('pac-nasc',     pac.dataNasc);
  set('pac-tel',      pac.tel);
  set('pac-cpf',      pac.cpf);
  set('pac-email',    pac.email);
  set('pac-diag',     pac.diag);
  set('pac-obs',      pac.obs);
...
function detalhePaciente(id) {
  ...
  bodyEl.innerHTML = `
    <div class="detail-section">
      <div style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:0.75rem;font-weight:600;background:${cor}20;color:${cor};margin-bottom:12px;">${labelConvenio(pac.convenio)}</div>
  ...
  if (btnEditar) {
    ...
    btnEditar.onclick = () => { closeModal('modal-detalhe-pac'); editarPaciente(id); };
  }
  openModal('modal-detalhe-pac');
}
...
function editarAnamnese(id) {
  ...
  const set = (elId, val) => { const el = document.getElementById(elId); if (el) el.value = val||''; };
  set('an-id',        a.id);
  set('an-pac',       a.pacienteId);
  set('an-data',      a.data);
  set('an-diag',      a.diag);
  set('an-queixa',    a.queixa);
  set('an-hda',       a.hda);
  set('an-pessoais',  a.pessoais);
  set('an-meds',      a.meds);
  set('an-objetivos', a.objetivos);
  openModal('modal-anamnese');
}
...
// ─── AGENDA — agrupada por paciente + seleção múltipla ───────────────────
let agendaFiltro   = 'ativos';
let agendaModoSel  = false;
const agSelecionados = new Set();
...
function renderAgenda() {
  ...
  el.innerHTML = pacIds.map(pacId => {
    ...
    const todosIds = sessoes.map(s => s.id);
    const todosSel = todosIds.every(id => agSelecionados.has(id));

    return `
      <div class="pac-grupo">
        <div class="pac-grupo-header" style="border-left:4px solid ${cor};">
          ${agendaModoSel ? `
            <input type="checkbox" class="sel-check sel-grupo" title="Selecionar todos"
              ${todosSel?'checked':''} onchange="toggleGrupoSel('${pacId}',this.checked)"
              onclick="event.stopPropagation()" style="width:18px;height:18px;cursor:pointer;flex-shrink:0;">
          ` : ''}
          <div style="flex:1;display:flex;align-items:center;gap:10px;cursor:pointer;" onclick="toggleGrupo('grupo-${pacId}')">
            ...
            <div class="pac-grupo-meta">
              ...
              <span class="pac-grupo-total">${sessoes.length} sessão${sessoes.length===1?'':'ões'}</span>
        ...
        <div class="pac-grupo-sessoes" id="grupo-${pacId}">
          ${sessoes.map(ag => {
            ...
            const sel     = agSelecionados.has(ag.id);
            return `
            <div class="sessao-item sessao-${ag.status} ${sel?'sessao-selecionada':''}"
                   onclick="${agendaModoSel ? `toggleSessaoSel('${ag.id}')` : `visualizarAgendamento('${ag.id}')`}">
                ${agendaModoSel ? `
                  <input type="checkbox" class="sel-check" ${sel?'checked':''} style="width:16px;height:16px;flex-shrink:0;"
                    onclick="event.stopPropagation()" onchange="toggleSessaoSel('${ag.id}')">
                ` : ''}
  ...
  document.querySelectorAll('.pac-grupo-sessoes.aberto').forEach(el => {
...
function ativarModoSelecao() {
  agendaModoSel = true;
  agSelecionados.clear();
  renderAgenda();
  document.querySelectorAll('.pac-grupo-sessoes').forEach(el => {
...
function toggleSessaoSel(agId) {
  if (agSelecionados.has(agId)) agSelecionados.delete(agId);
  else agSelecionados.add(agId);
  atualizarBarraSelecao();
  renderAgenda();
}

function toggleGrupoSel(pacId, checked) {
  const ags = DB.get('agendamentos');
  const filtered = ags.filter(a =>
    a.pacienteId === pacId &&
    (agendaFiltro==='ativos' ? STATUS_ATIVOS.includes(a.status) : !STATUS_ATIVOS.includes(a.status))
  );
  filtered.forEach(ag => {
    if (checked) agSelecionados.add(ag.id);
    else agSelecionados.delete(ag.id);
  });
  atualizarBarraSelecao();
  renderAgenda();
}
...
function atualizarBarraSelecao() {
  ...
  if (!barra) return;
  const n = agSelecionados.size;
  barra.style.display = agendaModoSel ? 'flex' : 'none';
  const contador = document.getElementById('sel-contador');
  if (contador) contador.textContent = n === 0 ? 'Nenhuma sessão selecionada' : `${n} sessão${n>1?'ões':''} selecionada${n>1?'s':''}`;
  const btnLote = document.getElementById('sel-btns-lote');
  if (btnLote) btnLote.style.display = n > 0 ? 'flex' : 'none';
}

function editarHorarioLote() {
  if (!agSelecionados.size) return;
  const horaEl = document.getElementById('lote-hora');
  if (horaEl) horaEl.value = '';
  const modal = document.getElementById('modal-lote-horario');
  if (modal) { modal.classList.remove('hidden'); modal.style.display = ''; }
}

function confirmarEditarHorarioLote() {
  const hora = document.getElementById('lote-hora')?.value;
  if (!hora) { toast('Informe o novo horário!'); return; }
  const ags = DB.get('agendamentos');
  let count = 0;
  agSelecionados.forEach(id => {
    const idx = ags.findIndex(a => a.id === id);
    if (idx !== -1) { ags[idx].hora = hora; count++; }
  });
  DB.set('agendamentos', ags);
  fecharModalLote();
  cancelarModoSelecao();
  renderAgenda(); renderDashboard();
  toast(`Horário atualizado em ${count} sessão${count>1?'ões':''}!`);
}

function excluirLote() {
  const n = agSelecionados.size;
  if (!n) return;
  if (!confirm(`Excluir ${n} sessão${n>1?'ões':''} selecionada${n>1?'s':''}? Esta ação não pode ser desfeita.`)) return;
  const ids = new Set(agSelecionados);
  DB.set('agendamentos', DB.get('agendamentos').filter(a => !ids.has(a.id)));
  cancelarModoSelecao();
  renderAgenda(); renderDashboard();
  toast(`${n} sessão${n>1?'ões':''} excluída${n>1?'s':''}!`);
}

function alterarStatusLote(novoStatus) {
  const n = agSelecionados.size;
  if (!n) return;
  const ags = DB.get('agendamentos');
  agSelecionados.forEach(id => {
    const idx = ags.findIndex(a => a.id === id);
    if (idx !== -1) ags[idx].status = novoStatus;
  });
  DB.set('agendamentos', ags);
  cancelarModoSelecao();
  renderAgenda(); renderDashboard();
  toast(`Status alterado em ${n} sessão${n>1?'ões':''}!`);
}
...
function renderEvolucoes() {
  ...
  if (!evs.length) {
    el.innerHTML = '<div class="empty-state">Nenhuma evolução cadastrada</div>';
    return;
  }

  const grupos = {};
  evs.forEach(e => {
    const key = e.pacienteId || '__sem_pac__';
    if (!grupos[key]) grupos[key] = [];
    grupos[key].push(e);
  });
  ...
  el.innerHTML = pacIds.map(pacId => {
    ...
    const cor  = convenioCor(pac?.convenio);
    const nome = pac?.nome || 'Paciente não encontrado';
    const evolucoes = grupos[pacId].sort((a,b) => (b.data||'').localeCompare(a.data||''));
    ...
    return `
      <div class="pac-grupo">
        <div class="pac-grupo-header" style="border-left:4px solid ${cor};">
          <div style="flex:1;display:flex;align-items:center;gap:10px;cursor:pointer;" onclick="toggleGrupoEv('ev-grupo-${pacId}')">
            <div class="pac-grupo-avatar" style="background:${cor}20;color:${cor};">${initials(nome)}</div>
            <div class="pac-grupo-info">
              <span class="pac-grupo-nome">${nome}</span>
              <span class="convenio-badge" style="background:${cor}20;color:${cor};">${labelConvenio(pac?.convenio)}</span>
            ...
            <div class="pac-grupo-meta">
              <span class="pac-grupo-badge" style="background:${cor};">${total} evolução${total===1?'':'ões'}</span>
              <span class="pac-grupo-total">Última: ${fmtDate(ultima?.data)}</span>
              <span class="pac-grupo-chevron" id="ev-chev-${pacId}">▾</span>
            </div>
          </div>
        </div>
        <div class="pac-grupo-sessoes" id="ev-grupo-${pacId}">
          ${evolucoes.map(e => `
            <div class="sessao-item" onclick="visualizarEvolucao('${e.id}')">
              <div class="sessao-data">
                <span class="sessao-dia">${fmtDate(e.data)}</span>
                <span class="sessao-hora">Sessão ${e.sessao||'—'}</span>
              </div>
              <div class="sessao-info">
                <span class="sessao-tipo">EVA ${e.eva!=null?e.eva:'—'}/10</span>
                <span class="sessao-prof">${(e.descricao||e.subj||e.texto||'—').substring(0,40)}${(e.descricao||e.subj||e.texto||'').length>40?'…':''}</span>
              </div>
              <div class="sessao-acoes" onclick="event.stopPropagation()">
...
function toggleGrupoEv(grupoId) {
  const el = document.getElementById(grupoId);
  if (!el) return;
  el.classList.toggle('aberto');
  const pacId = grupoId.replace('ev-grupo-', '');
  const chevron = document.getElementById('ev-chev-' + pacId);
  if (chevron) chevron.style.transform = el.classList.contains('aberto') ? 'rotate(180deg)' : '';
}

function novaEvolucao() {
  ['ev-id','ev-pac','ev-data',
   'ev-pa-ini','ev-spo2-ini','ev-bpm-ini',
   'ev-descricao',
   'ev-pa-fin','ev-spo2-fin','ev-bpm-fin',
   'ev-sessao','ev-eva','ev-subj','ev-obj','ev-aval','ev-plano','ev-texto'
  ].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  openModal('modal-evolucao');
}
function addEscala(tipo) {
  const campo = document.getElementById('ev-descricao');
  if (!campo) return;
  const escalas = {
    berg: "--- ESCALA DE EQUILÍBRIO DE BERG ---\n1. Sentado p/ em pé:\n2. Em pé sem apoio:\n3. Sentado sem apoio:\n4. Em pé p/ sentado:\n5. Transferências:\n6. Em pé (olhos fechados):\n7. Em pé (pés juntos):\n8. Alcançar a frente:\n9. Pegar objeto no chão:\n10. Virar p/ olhar para trás:\n11. Girar 360 graus:\n12. Pé no degrau:\n13. Em pé c/ um pé a frente:\n14. Em pé num pé só:\nPontuação Total:  /56\n",
...
function editarEvolucao(id) {
  ...
  set('ev-id',        ev.id);
  set('ev-pac',       ev.pacienteId);
  set('ev-data',      ev.data);
  set('ev-pa-ini',    ev.paIni);
  set('ev-spo2-ini',  ev.spo2Ini);
  set('ev-bpm-ini',   ev.bpmIni);
  set('ev-descricao', ev.descricao || ev.subj || ev.texto || '');
  set('ev-pa-fin',    ev.paFin);
  set('ev-spo2-fin',  ev.spo2Fin);
  set('ev-bpm-fin',   ev.bpmFin);
  set('ev-sessao',    ev.sessao);
  set('ev-eva',       ev.eva);
  set('ev-subj',      ev.subj || ev.texto);
  set('ev-obj',       ev.obj);
  set('ev-aval',      ev.aval);
  set('ev-plano',     ev.plano);
...
function salvarEvolucao() {
  ...
  if (!descricao.trim()) { toast('Preencha a descrição da evolução!'); return; }
  ...
  const obj = {
    ...
    pacienteId: pacId,
    data,
    paIni:   document.getElementById('ev-pa-ini')?.value   || '',
    spo2Ini: document.getElementById('ev-spo2-ini')?.value || '',
    bpmIni:  document.getElementById('ev-bpm-ini')?.value  || '',
    descricao,
    paFin:   document.getElementById('ev-pa-fin')?.value   || '',
    spo2Fin: document.getElementById('ev-spo2-fin')?.value || '',
    bpmFin:  document.getElementById('ev-bpm-fin')?.value  || '',
    subj:  descricao,
    texto: descricao,
    sessao: '',
    eva: null,
...
function visualizarEvolucao(id) {
  ...
  const sv = (val, unit) => val ? '<strong>' + val + '</strong> ' + unit : '<span style="color:var(--text-muted)">—</span>';

  const descricao = e.descricao || e.subj || e.texto || '—';
  const temSinais = !!(e.paIni || e.spo2Ini || e.bpmIni || e.paFin || e.spo2Fin || e.bpmFin);

  const blocoSinaisIni = temSinais ? (
    '<div class="ev-view-sinais" style="margin-bottom:12px;">' +
      '<div class="ev-view-label">📊 Sinais Vitais Iniciais</div>' +
      '<div class="ev-view-grid">' +
        '<div class="ev-view-item"><span class="ev-view-key">PA</span><span class="ev-view-val">' + sv(e.paIni, 'mmHg') + '</span></div>' +
        '<div class="ev-view-item"><span class="ev-view-key">SpO₂</span><span class="ev-view-val">' + sv(e.spo2Ini, '%') + '</span></div>' +
        '<div class="ev-view-item"><span class="ev-view-key">FC</span><span class="ev-view-val">' + sv(e.bpmIni, 'bpm') + '</span></div>' +
      '</div>' +
    '</div>'
  ) : '';

  const blocoSinaisFin = temSinais ? (
    '<div class="ev-view-sinais">' +
      '<div class="ev-view-label">📊 Sinais Vitais Finais</div>' +
      '<div class="ev-view-grid">' +
        '<div class="ev-view-item"><span class="ev-view-key">PA</span><span class="ev-view-val">' + sv(e.paFin, 'mmHg') + '</span></div>' +
        '<div class="ev-view-item"><span class="ev-view-key">SpO₂</span><span class="ev-view-val">' + sv(e.spo2Fin, '%') + '</span></div>' +
        '<div class="ev-view-item"><span class="ev-view-key">FC</span><span class="ev-view-val">' + sv(e.bpmFin, 'bpm') + '</span></div>' +
      '</div>' +
    '</div>'
  ) : '';

  const blocoLegado = (!temSinais && (e.obj || e.aval || e.plano)) ? (
    (e.obj  ? '<div class="mt-8"><label style="font-size:.75rem;color:var(--text-muted);font-weight:600;">OBJETIVO (O)</label><div>'   + e.obj.replace(/\n/g,'<br>')  + '</div></div>' : '') +
    (e.aval ? '<div class="mt-8"><label style="font-size:.75rem;color:var(--text-muted);font-weight:600;">AVALIAÇÃO (A)</label><div>'  + e.aval.replace(/\n/g,'<br>') + '</div></div>' : '') +
    (e.plano? '<div class="mt-8"><label style="font-size:.75rem;color:var(--text-muted);font-weight:600;">CONDUTA (P)</label><div>'    + e.plano.replace(/\n/g,'<br>')+ '</div></div>' : '')
  ) : '';

  document.getElementById('detalhe-pac-nome').textContent = 'Evolução — ' + (pac ? pac.nome : 'N/D');
  document.getElementById('detalhe-pac-body').innerHTML =
    '<div class="detail-section">' +
      '<div style="font-size:.8rem;color:var(--text-muted);margin-bottom:12px;">📅 ' + fmtDate(e.data) + '</div>' +
      blocoSinaisIni +
      '<div style="margin-bottom:12px;">' +
        '<div class="ev-view-label">📋 Evolução</div>' +
        '<div style="line-height:1.7;font-size:.9rem;white-space:pre-wrap;">' + descricao + '</div>' +
      '</div>' +
      blocoSinaisFin +
      blocoLegado +
    '</div>';
  ...
  const btn = document.getElementById('btn-editar-pac');
  if (btn) { btn.textContent = 'Editar Evolução'; btn.onclick = function() { closeModal('modal-detalhe-pac'); editarEvolucao(id); }; }
  openModal('modal-detalhe-pac');
}
...
function editarProfissional(id) {
  ...
  const set = (elId, val) => { const el = document.getElementById(elId); if (el) el.value = val||''; };
  set('prof-id',     p.id);
  set('prof-nome',   p.nome);
  set('prof-email',  p.email);
  set('prof-senha',  p.senha);
  set('prof-crefito',p.crefito);
  set('prof-tel',    p.tel);
  set('prof-esp',    p.esp);
  set('prof-perfil', p.perfil||'fisio');
  openModal('modal-profissional');
}
...
// Relatórios mockup function to ensure no error
function gerarRelatorio(tipo) {
  const dataIni = document.getElementById('rel-data-ini').value;
  const dataFim = document.getElementById('rel-data-fim').value;
  
  if (!dataIni || !dataFim) {
    toast('Selecione as datas inicial e final!');
    return;
  }

  const out = document.getElementById('relatorio-output');
  let conteudo = `<h3>Relatório de ${tipo.toUpperCase()} (${fmtDate(dataIni)} a ${fmtDate(dataFim)})</h3>`;
  
  // Aqui você mapeia a array correspondente (ex: DB.get('evolucoes')) 
  // filtra pelas datas e constrói uma <table class="rel-table">.
  conteudo += `<div class="empty-state">Gerando relatório de ${tipo}...</div>`;
  
  out.innerHTML = `<div id="pdf-content" style="padding:20px;">${conteudo}</div>
                   <button class="btn-primary mt-16" onclick="imprimirPDF()">Imprimir PDF</button>`;
}

function imprimirPDF() {
  const el = document.getElementById('pdf-content');
  html2pdf().set({ margin: 10, filename: `Relatorio_Limavi.pdf`, html2canvas: { scale: 2 } }).from(el).save();
}
...
function populateSelects() {
  ...
  const profOpts = '<option value="">Selecione o profissional</option>' +
    profs.map(p => `<option value="${p.id}">${p.nome} (${p.esp||p.perfil})</option>`).join('');
...
function exportarBackup() {
  const chaves = ['profissionais', 'pacientes', 'agendamentos', 'evolucoes', 'anamneses', 'limavi_init'];
  const backup = {};
  ...
  a.href = url;
  a.download = `limavi_backup_\${fmtDateISO(new Date())}.json`;
...
function importarBackup(event) {
  ...
  reader.onload = function(e) {
    try {
      ...
      alert('Erro ao ler o arquivo de backup. Verifique se é o arquivo correto.');
    }
  };
