...
// ─── DASHBOARD ─────────────────────────────────────────────────────────────
let dashCalendarDate = new Date(); // mês exibido no calendário
...
// ── Calendário mensal no Dashboard
function renderDashCalendar() {
  ...
  // Dias com agendamento neste mês
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const classes   = ['cal-day', isToday ? 'today' : '', hasAg ? 'has-ag' : ''].filter(Boolean).join(' ');
    const onclick   = hasAg ? `onclick="verAgendamentosData('${dateISO}')"` : '';
    html += `<div class="${classes}" ${onclick}>${d}${hasAg ? '<span class="cal-dot"></span>' : ''}</div>`;
  }
...
// Abre visualização dos agendamentos de uma data específica
function verAgendamentosData(dateISO) {
  const ags  = DB.get('agendamentos').filter(a => a.data === dateISO);
  const pacs = DB.get('pacientes');
  ...
  nomeEl.textContent = 'Agendamentos — ' + fmtDate(dateISO);

  if (!ags.length) {
    bodyEl.innerHTML = '<div class="empty-state">Sem agendamentos neste dia.</div>';
  } else {
    const tipos = { avaliacao:'Avaliação', sessao:'Sessão', retorno:'Retorno', alta:'Alta' };
    bodyEl.innerHTML = ags
      ...
      .map(ag => {
        ...
        return `
          <div class="list-card" style="border-left:4px solid ${cor};margin-bottom:8px;" onclick="visualizarAgendamento('${ag.id}')">
            <div style="min-width:48px;text-align:center;font-weight:700;color:var(--primary)">${ag.hora}</div>
            <div class="card-body">
              <div class="card-name">${pac?.nome || 'N/D'}</div>
              <div class="card-sub">${tipos[ag.tipo]||ag.tipo||'Sessão'} · ${prof?.nome||'—'} · ${statusTag(ag.status)}</div>
            </div>
          </div>`;
      }).join('');
  }
  ...
  if (btnEditar) {
    btnEditar.textContent = 'Ir para Agenda';
    btnEditar.onclick = function() { closeModal('modal-detalhe-pac'); navigate('agenda'); };
  }
...
function renderProximosAgendamentos() {
  ...
  const proximos = ags
    .filter(a => {
      if (!a.data) return false;
      const d = new Date(a.data + 'T00:00:00');
      return d >= today && d <= weekEnd && a.status !== 'cancelado';
    })
    .sort((a,b) => (a.data+a.hora).localeCompare(b.data+b.hora));

  if (!proximos.length) {
    el.innerHTML = '<div class="empty-state">Nenhum agendamento nos próximos 7 dias.</div>';
    return;
  }
  ...
  el.innerHTML = proximos.map(ag => {
    ...
    return `
      <div class="list-card proximos-card" onclick="visualizarAgendamento('${ag.id}')"
           style="cursor:pointer;border-left:4px solid ${cor};">
        <div class="prox-date ${isToday?'prox-today':''}">
...
function convenioCor(conv) {
  return CONVENIOS[conv]?.cor || CONVENIOS.outro.cor;
}
function labelConvenio(conv) {
  return CONVENIOS[conv]?.label || '—';
}
...
function renderAgenda() {
  ...
  el.innerHTML = sorted.length
    ? sorted.map(a => {
        ...
        const cor  = convenioCor(a.convenio || pac?.convenio);
        return `
          <div class="agenda-slot" onclick="visualizarAgendamento('${a.id}')"
               style="cursor:pointer;border-left:4px solid ${cor};">
            <div class="slot-info">
              <span class="slot-time">${fmtDate(a.data)} ${a.hora}</span>
              <span class="slot-name">${pac?.nome||'N/D'}
                <span class="convenio-badge" style="background:${cor}20;color:${cor};">${labelConvenio(a.convenio||pac?.convenio)}</span>
              </span>
              <span class="slot-sub">${tipos[a.tipo]||'Sessão'} · ${prof?.nome||'—'} · ${a.duracao||'50'}min · ${statusTag(a.status)}</span>
            </div>
...
// ── Modal de novo agendamento — limpa tudo incluindo recorrência
...
// Mostra/oculta campos de recorrência dinamicamente
...
function editarAgendamento(id) {
  ...
  const set = (elId, val) => { const el = document.getElementById(elId); if (el) el.value = val||''; };
  set('ag-id',         ag.id);
  set('ag-pac',        ag.pacienteId);
  set('ag-data',       ag.data);
  set('ag-hora',       ag.hora);
  set('ag-tipo',       ag.tipo||'sessao');
  set('ag-duracao',    ag.duracao||'50');
  set('ag-status',     ag.status||'agendado');
  set('ag-obs',        ag.obs);
  set('ag-convenio',   ag.convenio||'');
  set('ag-recorrencia','unico'); // ao editar, trata como sessão única
  atualizarRecorrencia();
  populateSelects();
  // Profissional: define depois do populateSelects
  const agProfEl = document.getElementById('ag-prof');
  if (agProfEl) agProfEl.value = ag.profId || currentUser?.id || '';
  openModal('modal-agendamento');
}

// Gera agendamentos recorrentes com base nas opções
function salvarAgendamento() {
  ...
  // ─── Recorrente: gera múltiplas datas ───────────────────────────────────
  ...
  // Evita duplicatas (mesmo paciente, mesma data/hora)
  let adicionados = 0;
  datas.forEach(d => {
